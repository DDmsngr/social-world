...
const dotColor = (s: Milestone['status']) =>
  s === 'now' ? '#D97757' : s === 'next' ? '#A8B89C' : 'rgba(236,231,222,0.35)'
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: m.status === 'now' ? '0 0 24px rgba(217,119,87,0.6)' : 'none',
                  // @ts-expect-error css var support
                  '--tw-ring-color': '#0a0608',
                }}
              />
          ...
          {milestones.map((m) => (
            <div key={`${m.year}-${m.quarter}-mob`} className="relative">
              <span
                ...
                style={{
                  background: dotColor(m.status),
                  boxShadow: m.status === 'now' ? '0 0 20px rgba(217,119,87,0.6)' : 'none',
                }}
              />
